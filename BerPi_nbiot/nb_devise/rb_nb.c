...
int qsdk_nb_sim_check(void)
{
    ...
    LOG_D("AT+CFUN?\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CFUN?") != RT_EOK)
        return RT_ERROR;
...
char *qsdk_nb_get_imsi(void)
{
    ...
    LOG_D("AT+CIMI\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CIMI") != RT_EOK)
        return RT_NULL;
...
char *qsdk_nb_get_imei(void)
{
    ...
    LOG_D("AT+CGSN=1\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CGSN=1") != RT_EOK)
        return RT_NULL;

    at_resp_parse_line_args(nb_resp, 2, "+CGSN:%s", nb_device_table.imei); //解析IMEI信息
...
static void qsdk_nb_set_rtc_time(int year, int month, int day, int hour, int min, int sec)
{
    ...
    year += 2000;
#endif
    rt_kprintf("当前时间:%d-%d-%d,%d-%d-%d,星期:%d\n", year, month, day, hour, min, sec, week);
...
int qsdk_nb_get_time(void)
{
    ...
    LOG_D("AT+CCLK?\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CCLK?") != RT_EOK)
        return RT_ERROR;
...
int qsdk_nb_get_csq(void)
{
    ...
    LOG_D("AT+CSQ\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CSQ") != RT_EOK)
        return -1;
...
int qsdk_nb_set_net_start(void)
{
    ...
    LOG_D("AT+CGATT=1\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CGATT=1") != RT_EOK)
        return RT_ERROR;
...
{
    if (nb_device_table.reboot_type == NB_MODULE_REBOOT_BY_PIN)
        return 1;
    else if (nb_device_table.reboot_type == NB_MODULE_REBOOT_BY_AT)
        return 2;
    else if (nb_device_table.reboot_type == NB_MODULE_REBOOT_BY_FOTA)
        return 3;
    else
        return 4;
}
...
int qsdk_nb_set_psm_mode(int mode, char *tau_time, char *active_time)
{
    if (mode == 0)
    {
        ...
        LOG_D("AT+CPSMS=0\n");
#endif
        if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CPSMS=0") != RT_EOK)
            return RT_ERROR;
    }
    else if (mode == 1)
    {
...
#endif
        if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CPSMS=1,,,\"%s\",\"%s\"", tau_time, active_time) != RT_EOK)
            return RT_ERROR;
...
#endif
    //等待退出PSM事件
    if (rt_event_recv(nb_event, EVENT_ENTER_RUN, RT_EVENT_FLAG_AND | RT_EVENT_FLAG_CLEAR, RT_WAITING_FOREVER,
                      &status) != RT_EOK)
    {
...
int qsdk_nb_ping_ip(char *ip)
{
    ...
    LOG_D("AT+NPING=%s\n", ip);
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+NPING=%s", ip) != RT_EOK)
        return RT_ERROR;
...
int qsdk_nb_set_rai_mode(int rai)
{
    ...
    LOG_D("AT*NBIOTRAI=%d\n", rai);
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT*NBIOTRAI=%d", rai) != RT_EOK)
        return RT_ERROR;
#else
#if (defined QSDK_USING_LOG)
    LOG_D("AT*MNBIOTRAI=%d\n", rai);
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT*MNBIOTRAI=%d", rai) != RT_EOK)
        return RT_ERROR;
#endif
    return RT_EOK;
}
...
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CMSYSCTRL=0,2") != RT_EOK)
        return RT_ERROR;
#else
#if (defined QSDK_USING_LOG)
    LOG_D("AT+ZCONTLED=1\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+ZCONTLED=1") != RT_EOK)
        return RT_ERROR;
#endif
    return RT_EOK;
}
...
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CMSYSCTRL=0,0") != RT_EOK)
        return RT_ERROR;
#else
#if (defined QSDK_USING_LOG)
    LOG_D("AT+ZCONTLED=0\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+ZCONTLED=0") != RT_EOK)
        return RT_ERROR;
#endif
    return RT_EOK;
}
...
int qsdk_nb_open_auto_psm(void)
{
    ...
    LOG_D("AT+SM=UNLOCK\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+SM=UNLOCK") != RT_EOK)
        return RT_ERROR;
#endif
    return RT_EOK;
}
...
int qsdk_nb_close_auto_psm(void)
{
    ...
    LOG_D("AT+SM=LOCK\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+SM=LOCK") != RT_EOK)
        return RT_ERROR;
#endif
    return RT_EOK;
}
...
char *qsdk_nb_query_ip(void)
{
    ...
    LOG_D("AT+CGPADDR\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+CGPADDR") != RT_EOK)
        return RT_NULL;
...
int qsdk_iot_check_address(void)
{
    ...
    LOG_D("AT+NCDP?\n");
#endif
    if (at_obj_exec_cmd(nb_client, nb_resp, "AT+NCDP?") != RT_EOK)
        return RT_ERROR;
    ...
    if (rt_strstr(str, QSDK_IOT_ADDRESS) != RT_NULL)
        return RT_EOK;
#else
    if (rt_strstr(str, "+NCDP:0.0.0.0") != RT_NULL)
        return RT_EOK;
#endif
...
void qsdk_psm_entry(void *parameter)
{
    ...
    {
        //等待进入PSM指示
        rt_event_recv(nb_event, EVENT_ENTER_PSM | EVENT_EXIT_PSM | EVENT_EXIT_PSM_REBOOT,
                      RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, RT_WAITING_FOREVER, &status);
...
int qsdk_init_environment(void)
{
    ...
    //create mail
    nb_mail = rt_mb_create("qsdk_mb", 10,
                           RT_IPC_FLAG_FIFO);
    if (nb_mail == RT_NULL)
    {
    ...
    qsdk_thread_id = rt_thread_create("qsdk_th", qsdk_thread_entry,
                                      RT_NULL, 1024 + 512, 7, 50);
    ...
    //create psm hand fun
    qsdk_psm_id = rt_thread_create("qsdk_psm", qsdk_psm_entry,
                                   RT_NULL, 512 + 200, 12, 50);
    ...
    if (argc > 1)
    {
        if (!strcmp(argv[1], "quick_connect"))
        {
            ...
            if (qsdk_nb_quick_connect() != RT_EOK)
                ...
                rt_kprintf("nb-iot module connect to Network success\n");
        }
        else if (!strcmp(argv[1], "reboot"))
        {
            if (qsdk_nb_reboot() != RT_EOK)
                rt_kprintf("nb-iot reboot error\n");
        ...
        else if (!strcmp(argv[1], "get_net_connect"))
        {
            if (qsdk_nb_get_net_connect() == RT_EOK)
                ...
                rt_kprintf("nb-iot module networking  error\n");
        }
